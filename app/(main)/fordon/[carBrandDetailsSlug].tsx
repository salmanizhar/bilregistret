...
import { CarFordonDetailsResponse, useCarModelDetailsBottomItems, useFordonCarModelDetails, useFordonCarModelDetailsBottomItems } from '@/Services/api/hooks/cars.hooks';
...
export default function CarBrandDetails() {
    const params = useLocalSearchParams();
    const { carBrandDetailsSlug } = params;
    ...
    const dynamicStyles = useMemo(() => ({
        ...
        imageItemContainer: {
            ...
            height: 190,
            borderRadius: 12,
            overflow: 'hidden' as const, // Fix linter error by explicitly typing as const
        },
    }), [layoutWidth]);
    ...
    const [modelDetails, setModelDetails] = useState<CarFordonDetailsResponse | null>(null);
    const shimmerAnim = useRef(new Animated.Value(0)).current;
    const fadeAnim = useRef(new Animated.Value(0)).current;

    const modelName = `${modelDetails?.C_merke} ${modelDetails?.C_modell} ${modelDetails?.C_typ}`;
    // Get the car model details mutation
    const carModelDetailsMutation = useFordonCarModelDetails();
    const { data: bottomItemsData, isLoading: isBottomItemsLoading, refetch: refetchBottomItems } = useFordonCarModelDetailsBottomItems(
        modelDetails?.merke_id || '',
        modelDetails?.C_modell || '',
        modelDetails?.modell_id || '',
        modelDetails?.Fordons_ar || '',
        { enabled: isFocused && !!modelDetails?.merke_id && !!modelDetails?.C_modell && !!modelDetails?.modell_id && !!modelDetails?.Fordons_ar }
    );
    ...
    const mergedCarSuggestionData = useMemo(() => {
        ...
        return (bottomItemsData ?? []).slice(0, maxItems).map((bottomItem: any, index: number) => {
            ...
            return {
                id: String(index + 1),
                image_url: convertWixImageUrl(Platform.OS === "web" ? carModel?.high_res || '' : carModel?.img_url || ''),
    ...
    let modelDetailsDescription = modelDetails?.C_merke + " " + modelDetails?.C_modell + " " + modelDetails?.C_typ + " med " +
        modelDetails?.C_bransle.toLowerCase() + " motor " + modelDetails?.C_kaross.toLowerCase() + " med " + modelDetails?.C_vaxellada.toLowerCase() +
        " växellåda som har en tjänstevikt på " + modelDetails?.Tjanstevikt + " kg. och totalvikt " +
        modelDetails?.Totalvikt + " kg."

    if (modelDetails?.Registreringsdatum !== null) {
        modelDetailsDescription += " och Registrerings datum " + `${modelDetails?.Registreringsdatum.toString().substring(0, 4)}-${modelDetails?.Registreringsdatum.toString().substring(4, 6)}-${modelDetails?.Registreringsdatum.toString().substring(6, 8)}`
    }
    ...
    // Fetch car model details when component mounts - improve with AbortController
    useEffect(() => {
        const controller = new AbortController();

        const fetchModelDetails = async () => {
            if (!carBrandDetailsSlug) {
                setIsError(true);
                setIsLoading(false);
                return;
            }
            ...
            try {
                const result = await carModelDetailsMutation.mutateAsync({
                    title: carBrandDetailsSlug as string
                });
                // // console.log("API result", result);
                if (controller.signal.aborted) return;

                setModelDetails(result[0]);
                setIsLoading(false);
            } catch (error) {
                if (controller.signal.aborted) return;
                setIsError(true);
        ...
        return () => {
            controller.abort();
            // Reset state on cleanup to prevent memory leaks
    ...
    const carImages = Platform.OS === "web" ? modelDetails?.high_res
        ? [modelDetails.high_res]
        : [
            'https://media.ed.edmunds-media.com/bmw/i4/2022/oem/2022_bmw_i4_sedan_m50_fq_oem_1_1600.jpg'
        ] : modelDetails?.img_url ? [modelDetails?.img_url] : [
            'https://media.ed.edmunds-media.com/bmw/i4/2022/oem/2022_bmw_i4_sedan_m50_fq_oem_1_1600.jpg'
        ];
        ...
        return [
            {
                ...
                value: modelDetails.Registreringsdatum ? `${modelDetails?.Registreringsdatum?.toString().substring(0, 4)}-${modelDetails?.Registreringsdatum?.toString().substring(4, 6)}-${modelDetails?.Registreringsdatum?.toString().substring(6, 8)}` : "-"
            },
            {
                icon: ImagePath.SvgIcons.FordonTillverkningsmanadIcon,
                label: "Tillverkningsmånad",
                value: modelDetails.Tillverkningsmanad ? `${modelDetails?.Tillverkningsmanad?.toString().substring(0, 4)}-${modelDetails?.Tillverkningsmanad?.toString().substring(4, 6)}` : "-"
            },
            {
            ...
            {
                icon: ImagePath.SvgIcons.FordonTotalviktIcon,
                label: "Totalvikt",
                value: `${modelDetails.Totalvikt} Kg` || "N/A"
            },
            {
                icon: ImagePath.SvgIcons.FordonVaxelladaIcon,
                label: "Växellåda",
                value: modelDetails.C_vaxellada || "N/A"
            },
            {
                icon: ImagePath.SvgIcons.FordonTjansteviktIcon,
                label: "Tjänstevikt",
                value: `${modelDetails.Tjanstevikt} Kg` || "N/A"
            },
        ];
    }, [modelDetails]);
    ...
    const handleShare = async () => {
        try {
            ...
            const shareUrl = `${baseUrl}/fordon/${carBrandDetailsSlug}`;
    ...
    const renderModelDetailItem = useCallback((item: ModelDetailItem, index: number) => (
        <View key={index} style={styles.detailItemContainer}>
            ...
            <View style={styles.detailTextContainer}>
                ...
                <MyText style={styles.detailValue}>{item.value}</MyText>
            </View>
        </View>
    ), []);
                    ...
                    >
                        <DesktopViewWrapper>
                            <Animated.View style={[styles.content, { opacity: fadeAnim }]}>
                                {/* Model Name */}
                                <MyText fontFamily="Poppins" style={styles.modelName}>
                                    {modelDetails?.C_merke} {modelDetails?.C_modell} {modelDetails?.C_typ}
                                    {/* {item.c_merke} {item.subModelName} */}
                                </MyText>
                                ...
                                <View style={styles.descriptionSection}>
                                    <MyText style={styles.descriptionText}>
                                        {modelDetailsDescription || "Ingen beskrivning tillgänglig."}
                                    </MyText>
                                </View>
                            </Animated.View>
                            <CarSuggestion
