...
import { View, StyleSheet, TouchableOpacity, FlatList, ScrollView, TouchableWithoutFeedback, Animated, Platform, Keyboard, InteractionManager, Dimensions, Easing, ActivityIndicator } from 'react-native';
...
import { useCarSubModels, useCarSubModelBottomItems } from '@/Services/api/hooks/car.hooks';
...
interface BottomItem {
    id: string;
    link_NYA_12: string;
    // Add other fields that may be in the API response
    title?: string;
    description?: string;
}
        ...
        return (
            <View style={styles.tableHeaderContainer}>
                <View style={styles.tableHeaderContentLayout0}>
                    ...
                    <View style={[styles.tableHeaderColumnLayout0, { left: 600 }]}>
...
const LayoutTextView = React.memo(({ item, onPress }: { item: CarSubModelData, onPress: () => void }) => {
    ...
    return (
        <TouchableOpacity
            style={styles.carSubModelItemContainer}
            onPress={onPress}
        >
            <MyText fontFamily='Poppins' style={styles.carSubModelName}>{item.title}</MyText>
            <View style={styles.carSubModelItemContent}>
                {/* <View style={[styles.carSubModelImage, { backgroundColor: myColors.screenBackgroundColor }]}>
                    {isUrl(item.imageUrl) && (
                        <Image
                            source={{ uri: item.imageUrl }}
                            style={styles.carSubModelImage}
                            contentFit="cover"
                            cachePolicy="disk"
                            transition={100}
                        />
                ...
                <View style={{ width: '70%' }}>
                    <ScrollView horizontal showsHorizontalScrollIndicator={false}>
                        <TouchableWithoutFeedback>
                            <View style={styles.carSubModelInfoRowContainer}>
                                {[...item.bodyTypes, item.C_chassi].map((data, index) => {
                                    if (data != null && data != "") {
                                        return (
                                            <View key={index} style={styles.carModelInfoContainer}>
                                                <MyText style={styles.carInfoText}>{data}</MyText>
                                            </View>
                                        )
                                    }
                                })}
                    ...
                    <ScrollView horizontal showsHorizontalScrollIndicator={false}>
                        <TouchableWithoutFeedback>
                            <View style={styles.engineTypeContainer}>
                                {[`${item.engine}`, ...item.C_motorkod].map((motorType, index) => (
                                    <View key={index} style={styles.carModelInfoContainer}>
                                        <MyText style={styles.carInfoText}>{motorType}</MyText>
                                    </View>
                                ))}
                            </View>
                        </TouchableWithoutFeedback>
                            ...
                            <View style={{ flexDirection: 'row', alignItems: 'center', gap: 5 }}>
                                <View style={styles.carSubModelInfoRowContainer}>
                                    {[...item.fuelTypes].map((fuelType, index) => {
                                        const iconXml = getSvgIcon(fuelType);
                                        return iconXml ? (
                                            <FuelTypeIconWithTooltip key={index} fuelType={fuelType} iconXml={iconXml} style={{ margin: 2.5, marginHorizontal: 0 }} />
                                        ) : null;
                                    })}
                                </View>
...
const LayoutGridtView = React.memo(({ item, onPress, index }: { item: CarSubModelData, onPress: () => void, index?: number }) => {
    const [imageLoaded, setImageLoaded] = useState(false);
    const isMounted = useRef(true);

    // WEB: Determine image priority based on position
    const isAboveFold = index !== undefined && index < 6; // First 6 sub-model images are critical
                ...
                <View style={styles.carBrandItemWrapperDesktopNew}>
                    ...
                    <View style={styles.layoutType1MiddleContainer}>
                        <View style={styles.carBrandItemContentDesktopNew}>
                            {/* Model Title */}
                            <MyText fontFamily='Inter' numberOfLines={1} style={styles.carModelNameDesktopNew}>
                                {item.title}
                            </MyText>

                            {/* Body Types and Engine Types Row */}
                            <View style={styles.bodyEngineTypesRowDesktopNew}>
                                {[...item.bodyTypes, item.C_chassi, `${item.engine}`, ...item.C_motorkod, ...item.engineTypes].slice(0, 4).map((type, index) => (
...
const LayoutLargeGridtView = React.memo(({ item, onPress, index }: { item: CarSubModelData, onPress: () => void, index?: number }) => {
    const [imageLoaded, setImageLoaded] = useState(false);

    // WEB: Determine image priority based on position for large images
    const isAboveFold = index !== undefined && index < 4; // First 4 large sub-model images are critical
    const priority = isAboveFold ? 'critical' : (index !== undefined && index < 10 ? 'high' : 'lazy');

    // WEB: Get optimized image props for large images (native gets standard props)
    const imageProps = isDesktopWeb()
        ? createOptimizedImageProps(item.imageUrl, 'detail', priority) // Use 'detail' for larger sub-model images
        : {
            source: { uri: item.imageUrl },
            priority: 'low' as const,
            contentFit: 'cover' as const,
    ...
    useEffect(() => {
        return () => {
            // WEB: Aggressive cleanup for large sub-model images
            if (isDesktopWeb() && isUrl(item.imageUrl)) {
    ...
    if (isDesktopWeb()) {
        return (
            <TouchableOpacity
                style={styles.carSubModelItemContainerLargeGrid}
                onPress={onPress}>
                <View>
                    <View style={[styles.carSubModelImageLargeGrid, { backgroundColor: myColors.screenBackgroundColor }]}>
                        {isUrl(item.imageUrl) && (
                            <Image
                                {...imageProps}
                                style={styles.carSubModelImageLargeGrid}
                            />
                        )}
                    </View>
                    <View style={styles.carSubModelItemContentLargeGrid}>
                        <MyText style={[styles.carSubModelNameLargeGrid, styles.desktopWebExtraSpace]}>{item.title}</MyText>
                        <View style={{ width: '100%' }}>
                            ...
                            >
                                <TouchableWithoutFeedback>
                                    <View style={styles.bodyTypeContainer}>
                                        {/* Combine body types and engine types like CarBrandSpecefic.tsx */}
                                        {[...item.bodyTypes, item.C_chassi, `${item.engine}`, ...item.C_motorkod, ...item.engineTypes].map((bodyType, index) => (
                                            <View key={index} style={styles.carModelInfoContainer}>
        ...
        return (
            <TouchableOpacity
                ...
                <View>
                    ...
                    <View style={styles.carSubModelItemContentLargeGrid}>
                        <MyText style={styles.carSubModelNameLargeGrid}>{item.title}</MyText>
                        <View style={{ width: '100%' }}>
                            <ScrollView
                                horizontal
                                showsHorizontalScrollIndicator={false}
...
const CarSubModel = () => {
    const { subBrand, brand, activeLayout } = useLocalSearchParams();
    // // console.log("props from sub brand", useLocalSearchParams());
    const c_modell = subBrand;
    const c_merke = brand;
    const isFocused = useIsFocused();
    // const { model_id, model, c_modell, c_merke, activeLayout } = useLocalSearchParams();
    ...
    const dynamicStyles = useMemo(() => ({
        ...
        // For example, if there are any card components that need responsive width
    }), [layoutWidth]);

    const [infoSearchInputText, setInfoSearchInputText] = useState('');

    // Initialize with layout from navigation params if available
    const initialLayout = activeLayout ? parseInt(activeLayout as string, 10) : 0;
    const { layoutType, updateLayoutType, isLoading: isLayoutLoading } = useLayoutType('model', initialLayout);
    ...
    const itemsPerPage = 10; // Optimized batch size for faster loading

    const [mergedCarSuggestionData, setMergedCarSuggestionData] = useState<any[]>([])
    const [mergedBottomSections, setMergedBottomSections] = useState<any[]>([])

    // Create an initial layout state to use for skeletons while waiting for layoutType to update
    const [initialLayoutType] = useState(initialLayout);
    ...
    const flatListRef = useRef<FlatList<CarSubModelData>>(null);
    ...
    const apiSubModelsData = !isDesktopWeb() || dataLoadingStrategy === 'API_ONLY' || dataLoadingStrategy === 'STATIC_WITH_FALLBACK'
        ? useCarSubModels(
            hyphensToSpaces(c_merke as string),
            hyphensToSpaces(c_modell as string),
            { enabled: isFocused }
        )
        : { data: [], isLoading: false, error: null }; // Dummy data only for SSG web

    // Step 3: Load sub-models data based on strategy
    const { data: carSubModels, isLoading: isCarSubModelsLoading, error } = useMemo(() => {
        switch (dataLoadingStrategy) {
            case 'MOBILE_API':
                ...
                return apiSubModelsData;

            case 'SSG_ONLY':
                // Pure SSG: Instant synchronous data, zero API calls (WEB ONLY)
                const ssgData = getSSGModelSubModels(spacesToHyphens(c_merke as string), spacesToHyphens(c_modell as string));
                // console.log('🚀 SSG_ONLY: Loaded sub-models instantly (WEB ONLY - zero API overhead)');
                return {
                    data: ssgData,
                    isLoading: false,
                    error: null
            ...
            case 'STATIC_WITH_FALLBACK':
                ...
                const modelData = getStaticModelData(spacesToHyphens(c_merke as string), spacesToHyphens(c_modell as string));
                if (modelData && modelData.subModels) {
                    // Transform the sub-models to match expected format
                    const transformedSubModels = modelData.subModels.map(subModel => ({
                        ID: subModel.id,
                    ...
                    // console.log('🌐 STATIC_WITH_FALLBACK: Using static sub-models (WEB)');
                    return {
                        data: transformedSubModels,
                        isLoading: false,
                        error: null
                    };
                } else {
            ...
            case 'API_ONLY':
                ...
                // console.log('📡 API_ONLY: Using API sub-models (WEB)');
                return apiSubModelsData;
    ...
    }, [dataLoadingStrategy, apiSubModelsData, c_merke, c_modell]);

    // Step 4: ALWAYS instantiate bottom items API hooks for mobile, conditionally for web
    const firstItem = carSubModels?.[0];
    const apiBottomItemsData = !isDesktopWeb() || dataLoadingStrategy === 'API_ONLY' || dataLoadingStrategy === 'STATIC_WITH_FALLBACK'
        ? useCarSubModelBottomItems(
            firstItem?.merke_id || '',
            firstItem?.C_modell || '',
            { enabled: isFocused && !!firstItem?.merke_id && !!firstItem?.C_modell }
        )
        : { data: [], isLoading: false, error: null }; // Dummy data only for SSG web

    // Bottom items data based on strategy
    const { data: bottomItemsData, isLoading: isBottomItemsLoading } = useMemo(() => {
        switch (dataLoadingStrategy) {
            case 'MOBILE_API':
                // Mobile ALWAYS uses API - this ensures mobile native works!
            ...
            case 'SSG_ONLY':
                // Pure SSG: Instant synchronous data, zero API calls (WEB ONLY)
                const ssgBottomItems = getSSGBottomItems(spacesToHyphens(c_merke as string), spacesToHyphens(c_modell as string));
            ...
            case 'STATIC_WITH_FALLBACK':
                // Static content with emergency API fallback (WEB ONLY)
                const staticBottomItems = getSSGBottomItems(spacesToHyphens(c_merke as string), spacesToHyphens(c_modell as string));
    ...
    }, [dataLoadingStrategy, apiBottomItemsData, c_merke, c_modell]);

    // Log strategy for debugging
    useEffect(() => {
        // console.log(`📊 CarBrandSpeceficSubModel Data Strategy: ${dataLoadingStrategy}`);
        ...
        if (dataLoadingStrategy === 'SSG_ONLY' && carSubModels && carSubModels.length > 0) {
            logSSGPerformance('CarBrandSpeceficSubModel', carSubModels.length);
        }
    }, [dataLoadingStrategy, carSubModels?.length]);

    const firstItemTitle = `${firstItem?.C_merke} ${firstItem?.C_modell}`;

    useEffect(() => {
        generateBottomItemsData();
    }, [firstItem, bottomItemsData]);

    const clearLocalStates = () => {
        setMergedCarSuggestionData([])
        setMergedBottomSections([])
    }
    ...
    // Start shimmer animation - optimized for faster loading feel
    useEffect(() => {
        if (isCarSubModelsLoading) {
            // Faster shimmer animation for better perceived performance
            Animated.loop(
    ...
    }, [isCarSubModelsLoading, shimmerAnim]);
    ...
    const generateBottomItemsData = () => {

        // Create merged car suggestion data
        const mergedCarSuggestionData = () => {
            if (!bottomItemsData || !carSubModels || carSubModels.length === 0) {
                return []; // Fallback to default data
            }

            // Limit to the available number of items (max 9)
            const maxItems = Math.min(bottomItemsData.length, 9);
            ...
            return bottomItemsData.slice(0, maxItems).map((bottomItem: any, index: number) => {
                ...
                const carModel = index < carSubModels.length ? carSubModels[index] : carSubModels[0];
        ...
        // // console.log("bottomItemsData", bottomItemsData);
        // // console.log("mergedCarSuggestionData", mergedCarSuggestionData);

        // Create bottom sections
        const mergedBottomSections = () => {
            if (!firstItem) {
                return []; // Fallback to default data
            }
            ...
            return [
                {
                    ...
                    description: (firstItem as any)["model description"] || `${firstItem?.C_merke || ''} ${firstItem?.C_modell || ''} - Ett tekniskt mästerverk`,
                },
                {
                    title: `ALLA MODELLER ${firstItem.C_merke} ${firstItem.C_modell}`,
                    description: (firstItem as any)["all model description"] || `${firstItem.C_merke} ${firstItem.C_modell} är en serie av bilar som tillverkas av ${firstItem.C_merke}`,
                }
            ];
        }

        setMergedCarSuggestionData(mergedCarSuggestionData())
        setMergedBottomSections(mergedBottomSections())
    ...
    }, [filterSlideAnim, fadeAnim]);

    // Create memoized item press handler to avoid recreation on every render
    const handleCarSubModelPress = useCallback((item: CarSubModelData) => {
        // // console.log("item from sub model", item);
        // router.push({
        //     pathname: '/(main)/CarBrandSpeceficModelDetails',
        //     params: {
        //         id: item.id,
        //         modell_id: item.modell_id,
        ...
        router.push({
            pathname: '/(main)/tillverkare/[brand]/[subBrand]/[subBrandDetails]',
            params: {
                brand: c_merke,
                subBrand: c_modell,
                subBrandDetails: item.slug.toLowerCase(),
            } as any
        });
    ...
    const ListEmptyView = useCallback(() => (
        <View style={styles.centerContent}>
            <MyText>Inga bilmodeller hittades</MyText>
        </View>
    ), []);
    ...
    const LoadingSkeleton = useCallback(() => {
        ...
        const TextLayoutSkeleton = () => {
            ...
            // Mobile Layout
            return (
                <View style={styles.carSubModelItemContainer}>
        ...
        const LargeGridLayoutSkeleton = () => (
            <View style={styles.carSubModelItemContainerLargeGrid}>
                <View>
                    <View style={[styles.carSubModelImageLargeGrid, { backgroundColor: myColors.screenBackgroundColor }]}>
                        <ShimmerOverlay />
                    </View>
        ...
        const SkeletonComponent =
            initialLayoutType === 0 ? TextLayoutSkeleton :
                initialLayoutType === 1 ? GridLayoutSkeleton :
                    LargeGridLayoutSkeleton;

        // Standard vertical list - reduce skeleton items for faster loading
        return (
    ...
    // Enhanced render item - removed opacity animation to fix visibility issues
    const renderItem = useCallback(({ item, index }: { item: CarSubModelData, index: number }) => {
        const onPress = () => handleCarSubModelPress(item);
        ...
        return (
            <View style={{ marginBottom: isDesktopWeb() ? 5 : 0 }}>
                <ItemComponent item={item} onPress={onPress} index={index} />
            </View>
        );
    }, [layoutType, handleCarSubModelPress]);
    ...
    // Handle background press. On native, dismiss keyboard; on web, keep focus to avoid blur issue.
    ...
    // Clear image cache when screen loses focus or model changes
    useFocusEffect(
        useCallback(() => {
            // Clear cache when entering screen with new model
            Image.clearMemoryCache();
        ...
        }, [c_modell]) // Add model parameters as dependencies to clear cache when model changes
    );

    // Optimize data transformation with useMemo and simpler processing
    const transformedData = useMemo(() => {
        if (!carSubModels || carSubModels.length === 0) return [];

        // Pre-calculate current year once
        const currentYear = moment().year();

        return carSubModels.map(subModel => {
            if (!subModel) return null;

            // Use pre-split arrays or empty arrays to avoid repeated splitting
            const fuelTypes = subModel.C_bransle ? subModel.C_bransle.split(',').map((s: string) => s.trim()).filter(Boolean) : [];
            const bodyTypes = subModel.C_kaross ? subModel.C_kaross.split(',').map((s: string) => s.trim()).filter(Boolean) : [];
            const engineTypes = subModel.C_hjuldrift ? subModel.C_hjuldrift.split(',').map((s: string) => s.trim()).filter(Boolean) : [];
            const motorCodes = subModel.C_motorkod ? subModel.C_motorkod.split(',').map((s: string) => s.trim()).filter(Boolean) : [];

            // Simplified year parsing
            const minYear = parseInt(subModel.C_fran_ar?.split('-')[0] || '0', 10) || 0;
            const maxYear = parseInt(subModel.C_till_ar?.split('-')[0] || '0', 10) || currentYear;

            return {
                id: subModel.ID || '',
                modell_id: subModel.modell_id || '',
                subModelName: `${subModel.C_modell || ''} ${subModel.C_typ || ''}`.trim(),
                yearRange: `${subModel.C_fran_ar || ''} - ${subModel.C_till_ar || ''}`,
                c_merke: subModel.C_merke || '',
                title: `${subModel.C_merke} ${subModel.C_modell || ''} ${subModel.C_typ || ''}`.trim() || '',
                registeredCars: subModel.t_count || '0',
                seats: `${subModel.minSeats || '0'}-${subModel.maxSeats || '0'}`,
                imageUrl: convertWixImageUrl(subModel[Platform.OS === "web" ? "high_res" : "Car Image"] || ''),
                imageUrlLowRes: convertWixImageUrl(subModel["Car Image"] || ''),
                fuelTypes,
                bodyTypes,
                engineTypes,
                C_chassi: subModel.C_chassi || '',
                C_motorkod: motorCodes,
                engine: `${subModel.C_kw || '0'} kW (${subModel?.C_hk || 0} hk)`,
                minYear,
                maxYear,
                slug: subModel.Title || ''
            };
        }).filter(Boolean) as CarSubModelData[];
    }, [carSubModels]);

    // WEB ONLY: EXTREME image preloading optimization for sub-model images
    useEffect(() => {
        if (isDesktopWeb() && transformedData.length > 0) {
            ...
            // Preload critical sub-model images immediately for blazing fast display
            ...
            // console.log(`🚀 WEB: Preloading ${criticalImages.length} critical sub-model images, queuing ${allImages.length - 6} for smart lazy load`);
        }
    }, [transformedData]);

    // Optimize filtering with useMemo and simplified processing
    const filteredData = useMemo(() => {
        ...
        return transformedData.filter(item => {
            ...
            // Fuel type check
            if (hasFuelTypeFilter && !item.fuelTypes.some(fuelType => fuelType.toUpperCase() === fuelFilter)) {
                return false;
            }

            // Chassis check
            if (hasChassisFilter && !item.bodyTypes.some(bodyType => bodyType.toUpperCase() === chassisFilter)) {
                return false;
            }
    ...
    const loadMoreData = useCallback(() => {
        if (!isDesktopWeb() && !isLoadingMore && hasMoreData && !isCarSubModelsLoading) {
            setIsLoadingMore(true);
            // Reduced loading delay for faster pagination
            setTimeout(() => {
                setCurrentPage(prev => prev + 1);
                setIsLoadingMore(false);
            }, 50);
        }
    }, [isLoadingMore, hasMoreData, isCarSubModelsLoading]);
    ...
    // Loading more indicator component - shows when pagination is active
    const renderLoadingIndicator = () => {
        if (!isLoadingMore || isDesktopWeb()) return null;

        return (
            <View style={[styles.loadingFooter, { marginBottom: 20 }]}>
    ...
    // Footer component - only renders when all data has been loaded (hasMoreData = false)
    const renderFooter = () => {
        // Only show footer when there's no more data to load
        if (hasMoreData) return null;

        return (
            <View style={styles.footerContainer}>
                <View style={styles.carSuggestionContainer}>
                    <CarSuggestion
                        title={`BILREGISTRET ${firstItemTitle}`}
                        cars={mergedCarSuggestionData}
                        bottomSections={mergedBottomSections}
                    />
                </View>
            </View>
        );
    ...
    const getItemLayout = useCallback((data: any, index: number) => {
        ...
        switch (layoutType) {
            ...
            case 2: // Large grid layout
                itemHeight = 250; // Same for both desktop and mobile
                break;
            default:
    ...
    const keyExtractor = useCallback((item: CarSubModelData) => item.id, []);
    ...
    // Add a clean-up effect at component level
    useEffect(() => {
        // Reference to animation values
        const filterSlideAnimation = filterSlideAnim;
        const fadeAnimation = fadeAnim;
        const shimmerAnimation = shimmerAnim;
        ...
        return () => {
            ...
            if (isDesktopWeb()) {
                ...
                // console.log('🧹 WEB: Cleaned up aggressive sub-model image optimization caches');
            }

            // Reset animation values
            filterSlideAnimation.setValue(0);
            fadeAnimation.setValue(0);
            shimmerAnimation.setValue(0);

            // Clear image cache
            Image.clearMemoryCache();
            cleanupForScreen('CarBrandSpeceficSubModel');
            performCacheCleanup();
                ...
                <TouchableWithoutFeedback onPress={handleBackgroundPress}>
                    ...
                    >
                        <DesktopViewWrapper>
                            <FlatList
                                ref={flatListRef}
                                data={isCarSubModelsLoading ? [] : paginatedData}
                                ...
                                ListEmptyComponent={
                                    isCarSubModelsLoading ? <LoadingSkeleton /> :
                                ...
                                initialNumToRender={isDesktopWeb() ? 50 : 15}
                                ...
                                maxToRenderPerBatch={isDesktopWeb() ? 50 : 15}
                                windowSize={isDesktopWeb() ? 5 : 5}
...
const styles = StyleSheet.create({
    ...
    carSubModelItemContainerLargeGrid: {
        backgroundColor: myColors.white,
        borderRadius: 8,
    ...
    carSubModelNameLargeGrid: {
        fontSize: 16,
        fontWeight: 'bold',
        marginBottom: 3,
    },
    carSubModelItemContent: {
        flexDirection: 'row',
        // alignItems: 'center',
    },
    carSubModelItemContentLargeGrid: {
        padding: 5,
    },
    carSubModelImage: {
        width: 110,
    ...
    carSubModelInfoRowContainer: {
        flexDirection: 'row',
        alignItems: 'center',
        flexWrap: 'wrap',
        marginBottom: 5,
    },
    carSubModelInfoContainer: {
        flexDirection: 'row',
        alignItems: 'center',
        backgroundColor: myColors.screenBackgroundColor,
        paddingHorizontal: 5,
    ...
    skeletonContentLargeGrid: {
        padding: 5,
    },
    skeletonStableContainer: {
        padding: 0,
    },
    ...
    footerContainer: {
        minHeight: 100, // Reserve minimum space to prevent layout shifts
    },
    carSuggestionContainer: {
        // Container for car suggestions to ensure consistent layout
    },
    ...
    // New Layout 1 Desktop Styles - Based on provided design from CarBrandSpecefic.tsx
